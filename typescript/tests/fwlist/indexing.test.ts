...
describe("at", () =>
{
  test("empty", () =>
  {
    ...
    assert.equal( el().at(1), undefined );
    assert.equal( el().at(99), undefined );
  });
  ...
  test("standard", () =>
  {
    ...
    assert.equal( fwl().at(8)!.value, "shard" );
    assert.equal( fwl().at(9)!.value, "shard" );
    assert.isUndefined( fwl().at(10) );
    assert.isUndefined( fwl().at(99) );
  });

  test("negative-unsupported", () =>
  {
    assert.isUndefined( fwl().at(-1) );
    assert.isUndefined( fwl().at(-2) );
    assert.isUndefined( fwl().at(-99) );
  });
  ...
  test("small", () =>
  {
    ...
    assert.isDefined( fwl.at(0) );
    assert.equal( fwl.at(0)!.value, "qi");
    assert.isUndefined( fwl.at(1), undefined);
    assert.isUndefined( fwl.at(99), undefined);
  });
